...
exports.exec = async (Bastion, message, args) => {
  try {
    ...
    if (/choco(?:late)?[-_ ]?(?:bar)?[s]?/i.test(args.product)) {
      ...
      return Bastion.emit('error', Bastion.strings.error(message.guild.language, 'notFound'), 'The specified product was not found in the shop. To check the available products, run `Shop` command.', message.channel);
    }
    ...
    message.channel.send({
      embed: {
        ...
        description: `${message.author.tag}, you successfully bought ${args.amount} ${gifts[args.product][0]} for ${requiredBalance} Bitts.`
      }
    }).catch(e => {
      Bastion.log.error(e);
    });
...
exports.help = {
  ...
  userTextPermission: '',
  userVoicePermission: '',
  usage: 'buy <product> [-a amount]',
  example: [ 'buy icecream', 'buyGift chocolate -a 2' ]
};
